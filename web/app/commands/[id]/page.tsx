...
function CommandDetail({ command }: { command: CommandCatalogEntry }) {
  return (
    <div className="mt-6 space-y-4">
      ...
      <CommandGuidance command={command} />

      <WorkflowBreadcrumbs commandId={command.id} />
...
/**
 * The one guidance block: catalog Use when / Produces / Next step plus
 * the web mapping (surface, presets, remaining inputs), each stated once.
 */
function CommandGuidance({ command }: { command: CommandCatalogEntry }) {
  const mapping = commandWorkflowMapping(command);
  return (
    <section className="glass rounded-xl p-5">
      <div className="flex flex-wrap items-center gap-2">
        <h2 className="text-base font-semibold tracking-tight">Guidance</h2>
        <span
          className={
      ...
      <p className="mt-4 max-w-3xl text-sm leading-relaxed text-[var(--fg-2)]">
        {mapping.summary}
      </p>
