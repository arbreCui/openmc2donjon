...
class OpenMCFullCoreMinicaseExampleTests(unittest.TestCase):
    ...
    def test_smoke_script_uses_production_entrypoint(self) -> None:
        ...
        self.assertIn("--acceptance-preset mechanical", text)
        self.assertIn("--acceptance-min-completed-iterations 2", text)
        self.assertIn("require_artifact_metadata_alignment", text)
        self.assertIn("max_final_clipped_count", text)
        self.assertIn('solve["keff"]', text)
        self.assertIn('summary["final_solve"]["keff"]', text)
