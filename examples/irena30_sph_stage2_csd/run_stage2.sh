...
IRENA_SPH2_CASE="${IRENA_SPH2_CASE:-csd_int}"
export IRENA_SPH2_CASE
RUN_ROOT="${RUN_ROOT:-/private/tmp/openmc2donjon_irena_sph_stage2_$IRENA_SPH2_CASE}"
...
SPH_FREEZE_GROUPS="${SPH_FREEZE_GROUPS:-31}"
...
echo "== IRENA SPH Stage 2: colorset $IRENA_SPH2_CASE CE fine vs MG coarse =="
echo "run root: $RUN_ROOT"
echo "CE: $PARTICLES x $BATCHES ($INACTIVE inactive); MG: $MG_PARTICLES x $MG_BATCHES"
echo "SPH iterations: $SPH_ITERATIONS  damping: $SPH_DAMPING  target: $SPH_TARGET  floor: $SPH_FLUX_FLOOR_REL  clip: [$SPH_CLIP_MIN, $SPH_CLIP_MAX]"
mkdir -p "$OUT_DIR"
...
echo
echo "IRENA SPH Stage 2 ($IRENA_SPH2_CASE) complete:"
