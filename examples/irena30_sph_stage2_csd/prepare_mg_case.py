"""Prepare the OpenMC multi-group macro (coarse) run for IRENA SPH Stage 2.

Loads the continuous-energy statepoint into the same MGXS library used by
``export_recipe.py``, asks OpenMC to create the MG-mode model (each of the
seven colorset cells filled with its homogenized macro material), and writes
the MG case directory. Optionally applies an SPH sidecar from the previous
iteration to the OpenMC-native MGXS file.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.summary_json is not None:
        payload = {
            "schema": "openmc2donjon.irena30-sph-stage2-mg-macro.v1",
            "scatter_format": MG_MACRO_SCATTER_FORMAT,
            "histogram_bins": MG_MACRO_HISTOGRAM_BINS,
            "mgxs": str(mgxs_path),
