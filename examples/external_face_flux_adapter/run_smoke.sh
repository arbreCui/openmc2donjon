...
RUN_DIR="${RUN_DIR:-/private/tmp/openmc2donjon_external_face_flux_adapter}"
PYTHON_BIN="${PYTHON_BIN:-python3}"
FACES="FD_XMIN,FD_XMAX,FD_YMIN,FD_YMAX"

INPUT_DIR="$RUN_DIR/inputs"
MGXS="$INPUT_DIR/mgxs_library.h5"
RAW_FACE_FLUX="$INPUT_DIR/external_solver_raw_face_flux.h5"
...
echo "== openmc2donjon external face-flux adapter smoke =="
echo "repo: $REPO_ROOT"
...
"$PYTHON_BIN" "$REPO_ROOT/examples/external_face_flux_adapter/make_inputs.py" \
...
echo
echo "== Adapt external homogeneous face flux =="
"$PYTHON_BIN" "$REPO_ROOT/examples/external_face_flux_adapter/adapt_face_flux.py" \
  "$MGXS" "$RAW_FACE_FLUX" \
  -o "$HOMOGENEOUS_FACE_FLUX" \
  --dataset solver/face_flux \
  --layout MGF \
  --faces "$FACES" \
  --source-label "external face-flux adapter example" \
  --force
...
"$PYTHON_BIN" -m openmc2donjon.cli make-adf-sidecar "$MGXS" \
  ...
  --homogeneous-face-flux "$HOMOGENEOUS_FACE_FLUX::homogeneous_face_flux" \
  --faces "$FACES" \
  --adf-kind external-face-flux-example \
  --adf-real true \
  --adf-source-label "external face-flux adapter example" \
...
"$PYTHON_BIN" - "$REFERENCE" "$HOMOGENEOUS_FACE_FLUX" "$ADF_SIDECAR" "$AUGMENTED_H5" "$MCOMPO" "$MACROLIB" "$CHECK_SUMMARY" "$FACE_FLUX_CHECK_SUMMARY" "$ADF_SIDECAR_SUMMARY" "$ADF_AUGMENT_SUMMARY" <<'PY'
import json
...
with h5py.File(homogeneous_path, "r") as h5:
    ...
    if h5.attrs["schema"] != "openmc2donjon.external-face-flux.v1":
        raise SystemExit("adapted face-flux schema mismatch")
    if h5.attrs["source_raw_dataset"] != "solver/face_flux":
        raise SystemExit("adapted face-flux raw dataset provenance mismatch")

with h5py.File(sidecar_path, "r") as h5:
    np.testing.assert_allclose(h5["adf"][:], expected_adf)
    if h5.attrs["adf_kind"] != "external-face-flux-example":
...
if face_flux_summary["homogeneous_face_flux_dataset"] != "homogeneous_face_flux":
    raise SystemExit("canonical homogeneous-face-flux dataset was not selected")
...
echo
echo "openmc2donjon external face-flux adapter smoke: PASS"
