...
OPENMC_RUN_DIR="$CASE_DIR/openmc_export"
MGXS="$OPENMC_RUN_DIR/mgxs_library.h5"
SCAFFOLD_DIR="$CASE_DIR/sph_loop_inputs"
...
"$PYTHON_BIN" -m openmc2donjon.cli make-sph-loop-scaffold "$MGXS" \
...
"$PYTHON_BIN" - "$MGXS" "$SCAFFOLD_DIR" "$SCAFFOLD_SUMMARY" <<'PY'
...
assert summary["decision"] == "openmc2donjon_sph_loop_scaffold_passed"
