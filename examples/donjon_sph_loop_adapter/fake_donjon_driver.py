"""Deterministic stand-in for a DONJON solve/postprocess pair.

The example uses this script only to exercise the generic ``run-sph-loop``
contract.  A production case replaces these subcommands with real DONJON cards
that consume the current ASCII macrolib and dump scalar volume fluxes.
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Fake DONJON driver for the SPH loop adapter smoke."
    )
    ...
    apply = subparsers.add_parser("apply", help="copy the workflow output")
...
def _solve(macrolib: Path, result: Path, iteration: int) -> int:
    ...
    print(f"fake DONJON solve iteration={iteration} macrolib={macrolib}")
    print(f"fake DONJON flux dump={result}")
    return 0


def _apply(input_path: Path, output: Path, sph: Path, iteration: int) -> int:
    if not input_path.exists():
        raise SystemExit(f"missing workflow input: {input_path}")
    if not sph.exists():
        raise SystemExit(f"missing SPH sidecar: {sph}")
    output.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(input_path, output)
    print(f"fake DONJON postprocess iteration={iteration} output={output}")
    return 0
